...
TH1F* LW200SecVec_PpointS_2m = new TH1F("LW200SecVec_PpointS_2m","Secondary vertex total momentum Secondary vertex point sign dxy>0.02mm;Ppoint sign;",100,-2,2);
...
TH1F* LW200JetZdau_IP1 = new TH1F("LW200JetZdau_IP1","Zdau impact parameter to 1 secvec;Impact parameter[mm];",100,0,1);
TH1F* LW200JetZdau_IP2 = new TH1F("LW200JetZdau_IP2","Zdau impact parameter to 2 secvec;Impact parameter[mm];",100,0,1);

TH1F* LW200SecVec_disp = new TH1F("LW200SecVec_disp","Sec vec displacement weigthed;distance/sigma;",200,0,1);
...
public :
  ...
  vector<float> *Esecvec_jet_px;
  vector<float> *Esecvec_jet_py;
  vector<float> *Esecvec_jet_pz;
  vector<float> *Esecvec_jet_pt1;
  vector<float> *Esecvec_jet_eta1;
  vector<float> *Esecvec_jet_phi1;
  vector<float> *Esecvec_jet_pt2;
  vector<float> *Esecvec_jet_eta2;
  vector<float> *Esecvec_jet_phi2;
  vector<float> *Esecvec_jet_d1;
  vector<float> *Esecvec_jet_d2;
  vector<float> *Esecvec_jet_eleTag;
  ...
  TBranch *b_Esecvec_jet_px;
  TBranch *b_Esecvec_jet_py;
  TBranch *b_Esecvec_jet_pz;
  TBranch *b_Esecvec_jet_pt1;
  TBranch *b_Esecvec_jet_eta1;
  TBranch *b_Esecvec_jet_phi1;
  TBranch *b_Esecvec_jet_pt2;
  TBranch *b_Esecvec_jet_eta2;
  TBranch *b_Esecvec_jet_phi2;
  TBranch *b_Esecvec_jet_d1;
  TBranch *b_Esecvec_jet_d2;
  TBranch *b_Esecvec_jet_eleTag;
...
void EventLoopAnalysisTemplate::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("Esecvec_pz",&Esecvec_pz,&b_Esecvec_pz);
   ...
   fChain->SetBranchAddress("Esecvec_jet_px",&Esecvec_jet_px,&b_Esecvec_jet_px);
   fChain->SetBranchAddress("Esecvec_jet_py",&Esecvec_jet_py,&b_Esecvec_jet_py);
   fChain->SetBranchAddress("Esecvec_jet_pz",&Esecvec_jet_pz,&b_Esecvec_jet_pz);
   fChain->SetBranchAddress("Esecvec_jet_pt1",&Esecvec_jet_pt1,&b_Esecvec_jet_pt1);
   fChain->SetBranchAddress("Esecvec_jet_eta1",&Esecvec_jet_eta1,&b_Esecvec_jet_eta1);
   ...
   fChain->SetBranchAddress("Esecvec_jet_eta2",&Esecvec_jet_eta2,&b_Esecvec_jet_eta2);
   fChain->SetBranchAddress("Esecvec_jet_phi2",&Esecvec_jet_phi2,&b_Esecvec_jet_phi2);
   fChain->SetBranchAddress("Esecvec_jet_d1",&Esecvec_jet_d1,&b_Esecvec_jet_d1);
   fChain->SetBranchAddress("Esecvec_jet_d2",&Esecvec_jet_d2,&b_Esecvec_jet_d2);
   fChain->SetBranchAddress("Esecvec_jet_eleTag",&Esecvec_jet_eleTag,&b_Esecvec_jet_eleTag);
...
void EventLoopAnalysisTemplate::analysis()
{
  ...
  for(size_t x=0; x<GenDau_pt->size(); x++){
    ...
    if(GenDau_pdgId->at(x)==-11 && GenDau_mompdgId->at(x)==-556) Pindex=x;
  }
  ...
  if(Zbool){
    ...
    //LW200ZdauCharge->Fill(GenDau_pdgId->at(Zindex[0]));
    //cout<<GenDau_pdgId->at(Zindex[0])*GenDau_pdgId->at(Zindex[1])<<endl;

    size_t eleSecvecCount=0;
    for(size_t x=0; x<Esecvec_eleTag->size();x++){
      if(Esecvec_eleTag->at(x)!=-1) eleSecvecCount++;
    }
    ...
    for(size_t i=0; i<jet_pt->size(); i++){
      for(size_t k=0; k<jet_pt->size(); k++){
        if((genjet_DRscore->at(i)<0.5 && genjet_pt->at(i)==GenDau_pt->at(Zindex[0])) && (genjet_DRscore->at(k)<0.5 && genjet_pt->at(k)==GenDau_pt->at(Zindex[1]))){
          ZTE=ZTE+jet_e->at(i);
          ZTpx=ZTpx+jet_px->at(i);
          ZTpy=ZTpy+jet_py->at(i);
          ZTpz=ZTpz+jet_pz->at(i);
          RecoZdau.push_back(make_pair(corr_jet_pt->at(i),i));
          Zcount++;
          ZTch=ZTch*jet_ch->at(i);
          for(size_t j=0; j<Esecvec_jet_eta1->size(); j++){
            float disp;
            for(size_t l=0; l<Esecvec_px->size(); l++){
              if(Esecvec_eleTag->at(l)== Esecvec_jet_eleTag->at(j)){
                float dx = Esecvec_posx->at(l) - Bsp_x->at(0);
                float dy = Esecvec_posy->at(l) - Bsp_y->at(0);
                float dz = Esecvec_posz->at(l) - Bsp_z->at(0);
                disp= sqrt(dx*dx+dy*dy);
              }
              if(Esecvec_eleTag->at(l)!=-1){
                if(disp>0.02){
                  float Vx=Esecvec_posx->at(l)-PV_x->at(0);
                  float Vy=Esecvec_posy->at(l)-PV_y->at(0);
                  float Vz=Esecvec_posz->at(l)-PV_z->at(0);
                  float Ppoint= Esecvec_px->at(l)*Vx + Esecvec_py->at(l)*Vy + Esecvec_pz->at(l)*Vz;
            ...
            if(disp>0.02){
              if(Esecvec_jet_eta1->at(j)==jet_eta->at(i) && Esecvec_jet_pt1->at(j)==jet_pt->at(i) && Esecvec_jet_eta2->at(j)==jet_eta->at(k) && Esecvec_jet_pt2->at(j)==jet_pt->at(k)){
                if(genelec_DRscore->at(Esecvec_jet_eleTag->at(j))<0.1 && genelec_pt->at(Esecvec_jet_eleTag->at(j))==GenDau_pt->at(Eindex)){
                  LW200JetZdau_IP1->Fill(Esecvec_jet_d1->at(j)+Esecvec_jet_d2->at(j));
                  //cout<<"EleTag1: "<<Esecvec_jet_eleTag->at(j)<<" "<<Esecvec_jet_d1->at(j)<<" "<<Esecvec_jet_d2->at(j)<<endl;
                }
                else if(genelec_DRscore->at(Esecvec_jet_eleTag->at(j))<0.1 && genelec_pt->at(Esecvec_jet_eleTag->at(j))==GenDau_pt->at(Pindex)) {
                  LW200JetZdau_IP2->Fill(Esecvec_jet_d1->at(j)+Esecvec_jet_d2->at(j));
                  //cout<<"EleTag2: "<<Esecvec_jet_eleTag->at(j)<<" "<<Esecvec_jet_d1->at(j)<<" "<<Esecvec_jet_d2->at(j)<<endl;
                }
              }
            }
          }
  ...
  for(size_t l=0; l<Esecvec_px->size(); l++){
    ...
    float sigmax=Bsp_widthx->at(0)+Esecvec_poserrorx->at(l);
    float sigmay=Bsp_widthy->at(0)+Esecvec_poserrory->at(l);

    float disp= sqrt(dx*dx+dy*dy);
    float sigma = sqrt( sigmax*sigmax + sigmay*sigmay );

    LW200SecVec_disp->Fill(disp/sigma,theweight);
  }
 ...
 */
  ...
  TFile* hfile = new TFile("SecvecAnalysisLW200.root","RECREATE");
