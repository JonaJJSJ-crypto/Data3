...
/////////////////////////Generated Inva Mass/////////////////////////////////
  ...
  LW200Gen_EleJet_DRdiff->Fill(abs(GenEleDRTemp.at(1)-GenEleDRTemp.front()));*/
    ...
    if(Zbool){
      ...
      for(size_t i=0; i<jet_pt->size(); i++){
        if((genjet_DRscore->at(i)<0.5 && genjet_pt->at(i)==GenDau_pt->at(Zindex[0])) || (genjet_DRscore->at(i)<0.5 && genjet_pt->at(i)==GenDau_pt->at(Zindex[1]))){
          ...
          /*for(size_t j=0; j<secvec_jet_eta1->size(); j++){
            if(secvec_jet_eleTag->at(j)!=-1){
              if(secvec_jet_eta1->at(j)==jet_eta->at(i) && secvec_jet_pt1->at(j)==jet_pt->at(i)){
                LW200JetZdau_IP->Fill(secvec_jet_d1->at(j));
              }
            }
          }*/
    ...
    for(size_t x=0; x<secvec_posx->size(); x++){
      if(secvec_eleTag->at(x)!=-1 && secvec_jet_px->at(x)!=-1 && secvec_jet_d1->at(x)<0.1 && secvec_jet_d2->at(x)<0.1 ){
        Vx=secvec_posx->at(x)-PV_x->at(0);
        Vy=secvec_posy->at(x)-PV_y->at(0);
        Vz=secvec_posz->at(x)-PV_z->at(0);
        Ppoint= electron_px->at(secvec_eleTag->at(x))*Vx+electron_py->at(secvec_eleTag->at(x))*Vy+electron_pz->at(secvec_eleTag->at(x))*Vz;
    ...
    float pvx=0,pvy=0,pvz=0,ppx=0,ppy=0,ppz=0;
    for(size_t x=0; x<GenPart_pt->size(); x++){
      if( (GenPart_pdgId->at(x) == 11 && GenPart_mompdgId->at(x) == 556)
       || (GenPart_pdgId->at(x) == 23 && GenPart_mompdgId->at(x) == 556) ){
        ppx=ppx+GenPart_px->at(x);
        ppy=ppy+GenPart_py->at(x);
        ppz=ppz+GenPart_pz->at(x);
      }
    }
    for(size_t x=0; x<GenPart_pt->size(); x++){
    ...
    /////////////////Identification to signal jets

    for(size_t k = 0; k < electron_pt->size(); k++){
      size_t ZdauCand=0;
      size_t BZdauCand=0;
      bool RecoZdauFlag2=false;
      bool RecoZdauFlag1=false;
      for (size_t i = 0; i < jet_pt->size(); i++) {
        for (size_t j = i+1; j < jet_pt->size(); j++) {
          float E=jet_e->at(i)+jet_e->at(j);
          float px=jet_px->at(i)+jet_px->at(j);
          float py=jet_py->at(i)+jet_py->at(j);
          float pz=jet_pz->at(i)+jet_pz->at(j);
          ...
          py=py+electron_py->at(k);
          pz=pz+electron_pz->at(k);
          float Emass=sqrt(E*E-px*px-py*py-pz*pz);
          if(mass>80 && mass<160){
              float DR1=deltaR(jet_eta->at(i),jet_phi->at(i),electron_eta->at(k),electron_phi->at(k));
              float DR2=deltaR(jet_eta->at(j),jet_phi->at(j),electron_eta->at(k),electron_phi->at(k));
              EleJetDR=min(DR1,DR2);
            if(EleJetDR>0.01 && EleJetDR<8){
              ZdauCand++;
              LW200Electron_ZdauCand_DR->Fill(EleJetDR);
              LW200Electron_ZdauCand_InvMass->Fill(Emass);
              if(k==ElePtTemp.back().second){
                BZdauCand++;
                LW200BElectron_ZdauCand_DR->Fill(EleJetDR);
                LW200BElectron_ZdauCand_InvMass->Fill(Emass);
              }
              if(RecoZdau.size()>=2){
                if((RecoZdau.back().second == i && RecoZdau.end()[-2].second == j)
                || (RecoZdau.back().second == j && RecoZdau.end()[-2].second == i)) RecoZdauFlag2=true;

                else if(RecoZdau.back().second == i || RecoZdau.end()[-2].second == i
                || RecoZdau.end()[-2].second == j || RecoZdau.back().second == j) RecoZdauFlag2=true;
              }
      ...
      LW200Electron_ZdauCand_Count->Fill(ZdauCand);
      if(k==ElePtTemp.back().second) LW200BElectron_ZdauCand_Count->Fill(BZdauCand);

      if(RecoZdauFlag2) LW200Electron_ZdauCand_RecoZdauMatch->Fill(2);
      else if(RecoZdauFlag1) LW200Electron_ZdauCand_RecoZdauMatch->Fill(1);
      else LW200Electron_ZdauCand_RecoZdauMatch->Fill(0);
      if(k==ElePtTemp.back().second){
        if(RecoZdauFlag2) LW200BElectron_ZdauCand_RecoZdauMatch->Fill(2);
        else if(RecoZdauFlag1) LW200BElectron_ZdauCand_RecoZdauMatch->Fill(1);
        else LW200BElectron_ZdauCand_RecoZdauMatch->Fill(0);
      }
    }
    ///////////////compare to reco Zdau vector
