...
void EventLoopAnalysisTemplate::Loop()
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(jentry==16) {
      ...
      LW200SecVec_XYGen1 = new TH2F("LW200SecVec_XYGen1","SecVec XY Gen Distribution1",200,Bx,Tx,200,By,Ty);
      LW200SecVec_XYGen2 = new TH2F("LW200SecVec_XYGen2","SecVec XY Gen Distribution2",200,Bx,Tx,200,By,Ty);
      ...
      LW200SecVec_XYSelected1 = new TH2F("LW200SecVec_XYSelected1","SecVec XY Signal Distribution1",200,Bx,Tx,200,By,Ty);
          ...
          for (size_t i = 0; i < electron_pt->size(); i++) {
            ...
            if( genelec_DRscore->at(i)<0.1 && genelec_pt->at(i)==GenPart_pt->at(x) ){
              for(size_t j=0; j<Esecvec_posx->size(); j++){
                ...
                if(Esecvec_eleTag->at(j)==i && GenPart_pdgId->at(x)==11) {
                  LW200SecVec_XYSignal1->Fill(Esecvec_posx->at(j),Esecvec_posy->at(j),50);
                  if(RecoZdauIndex.size()>=2){
                    for(size_t k=0; k<RecoZdauIndex.size(); k++){
                      //for(size_t l=k+1; l<RecoZdauIndex.size(); l++){
                        if( Esecvec_jet_pt1->at(j)==jet_pt->at(k) || Esecvec_jet_pt2->at(j)==jet_pt->at(k) ){
                          LW200SecVec_XYSelected1->Fill(Esecvec_posx->at(j),Esecvec_posy->at(j),50);
                          LW200SecVec_XYSelected1->Fill(Bsp_x->at(0),Bsp_y->at(0),100);
                        }
                      //}
                    }
                  }
                  ...
                  LW200SecVec_XYSignal1->Fill(Bsp_x->at(0),Bsp_y->at(0),100);

                  SignalCount++;
                }
                if(Esecvec_eleTag->at(j)==i && GenPart_pdgId->at(x)==-11) {
                  LW200SecVec_XYSignal2->Fill(Esecvec_posx->at(j),Esecvec_posy->at(j),50);
                  //LW200SecVec_XYSignal2->Fill(Esecvec_posx->at(j)-Esecvec_poserrorx->at(j),Esecvec_posy->at(j)-Esecvec_poserrory->at(j),5);
                  //cout<<Esecvec_posx->at(j)<<' '<<Esecvec_posy->at(j)<<endl;
                  //cout<<Esecvec_posx->at(j)-Esecvec_poserrorx->at(j)<<' '<<Esecvec_posy->at(j)-Esecvec_poserrory->at(j)<<"\n\n";
                  LW200SecVec_XYSignal2->Fill(Bsp_x->at(0),Bsp_y->at(0),100);
                  SignalCount++;
                }
                LW200SecVec_XYSignal3->Fill(Bsp_x->at(0),Bsp_y->at(0),100);
                if(Esecvec_eleTag->at(j)==i) {
                  LW200SecVec_XYSignal4->Fill(Esecvec_posx->at(j),Esecvec_posy->at(j),50);
                  //LW200SecVec_XYSignal4->Fill(Esecvec_posx->at(j)-Esecvec_poserrorx->at(j),Esecvec_posy->at(j)-Esecvec_poserrory->at(j),10);
                  //cout<<Esecvec_posx->at(j)<<' '<<Esecvec_posy->at(j)<<endl;
                  //cout<<Esecvec_posx->at(j)-Esecvec_poserrorx->at(j)<<' '<<Esecvec_posy->at(j)-Esecvec_poserrory->at(j)<<"\n\n";
                  LW200SecVec_XYSignal4->Fill(Bsp_x->at(0),Bsp_y->at(0),100);
                  SignalCount++;
                }
              }
            }
 ...
 */
  ...
  sampleNames.insert(make_pair("DYJetsToLL",make_pair("LW200",1)));
