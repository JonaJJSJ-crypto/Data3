...
def main(variable):
    ...
    LW200 = getHistogram(tfile, "IPP_LW200", variable)

    W = getHistogram(tfile, "IPP_W1J", variable)
    W2J = getHistogram(tfile, "IPP_W2J", variable)
    W3J = getHistogram(tfile, "IPP_W3J", variable)
    W.Add(W2J)
    W.Add(W3J)

    TT = getHistogram(tfile, "IPP_TT", variable)

    ZLL = getHistogram(tfile, "IPP_ZLL", variable)

    #WW = getHistogram(tfile, "IPP_WW", variable)

    #WZ = getHistogram(tfile, "IPP_WZ", variable)

    #ttZ = getHistogram(tfile, "IPP_ttZ", variable)

    # Data
    data = getHistogram(tfile, "IPP_dataRunB", variable)
    dataRunC = getHistogram(tfile, "IPP_dataRunC", variable)
    data.Add(dataRunC)

    # Data-driven QCD estimation
    QCD = getHistogram(tfile, "IPP_dataRunB", variable, "_cr")
    QCDRunC = getHistogram(tfile, "IPP_dataRunC", variable, "_cr")
    QCD.Add(QCDRunC)
    for name in ["IPP_W1J", "IPP_W2J", "IPP_W3J", "IPP_TT", "IPP_ZLL"]:
...
#    for x in [TT, W, ZLL]:#, WW, WZ, ttZ]:
    ...
    if variable == "ppoint":
        # Simulation
        #ggH = getHistogram(tfile, "ggH", variable)
        Uncert_LW200 = getHistogram(tfile, "IPP_LW200", variable,"_uncert")

        Uncert_W = getHistogram(tfile, "IPP_W1J", variable,"_uncert")
        Uncert_W2J = getHistogram(tfile, "IPP_W2J", variable,"_uncert")
        Uncert_W3J = getHistogram(tfile, "IPP_W3J", variable,"_uncert")
        Uncert_W.Add(Uncert_W2J)
        Uncert_W.Add(Uncert_W3J)

        Uncert_TT = getHistogram(tfile, "IPP_TT", variable,"_uncert")

        Uncert_ZLL = getHistogram(tfile, "IPP_ZLL", variable,"_uncert")
        ...
        # Data
        Uncert_data = getHistogram(tfile, "IPP_dataRunB", variable,"_uncert")
        Uncert_dataRunC = getHistogram(tfile, "IPP_dataRunC", variable,"_uncert")
        Uncert_data.Add(Uncert_dataRunC)

        # Data-driven QCD estimation
        Uncert_QCD = getHistogram(tfile, "IPP_dataRunB", variable,"_uncert")
        Uncert_QCDRunC = getHistogram(tfile, "IPP_dataRunC", variable,"_uncert")
    ...
    if variable == "svd" or variable == "nsv" or variable == "ppoint" or variable == "BSdChi2norm":
        c.SetLogy()
        c.SaveAs("%s.pdf" % ("PDF/"+variable+"_7_IPP"))
        c.SaveAs("%s.png" % ("PNG/"+variable+"_7_IPP"))
    else:
        c.SaveAs("%s.pdf" % ("PDF/"+variable+"_7_IPP"))
        c.SaveAs("%s.png" % ("PNG/"+variable+"_7_IPP"))
