...
def main(variable):
    ...
    LW200 = getHistogram(tfile, "SG_LW200", variable)

    W = getHistogram(tfile, "SG_W1J", variable)
    W2J = getHistogram(tfile, "SG_W2J", variable)
    W3J = getHistogram(tfile, "SG_W3J", variable)
    W.Add(W2J)
    W.Add(W3J)

    TT = getHistogram(tfile, "SG_TT", variable)

    ZLL = getHistogram(tfile, "SG_ZLL", variable)

    #WW = getHistogram(tfile, "SG_WW", variable)

    #WZ = getHistogram(tfile, "SG_WZ", variable)

    #ttZ = getHistogram(tfile, "SG_ttZ", variable)

    # Data
    data = getHistogram(tfile, "SG_dataRunB", variable)
    dataRunC = getHistogram(tfile, "SG_dataRunC", variable)
    data.Add(dataRunC)

    # Data-driven QCD estimation
    QCD = getHistogram(tfile, "SG_dataRunB", variable, "_cr")
    QCDRunC = getHistogram(tfile, "SG_dataRunC", variable, "_cr")
    QCD.Add(QCDRunC)
    for name in ["SG_W1J", "SG_W2J", "SG_W3J", "SG_TT", "SG_ZLL"]:
...
#    for x in [TT, W, ZLL]:#, WW, WZ, ttZ]:
    ...
    if variable == "ppoint":
        # Simulation
        #ggH = getHistogram(tfile, "ggH", variable)
        Uncert_LW200 = getHistogram(tfile, "SG_LW200", variable,"_uncert")

        Uncert_W = getHistogram(tfile, "SG_W1J", variable,"_uncert")
        Uncert_W2J = getHistogram(tfile, "SG_W2J", variable,"_uncert")
        Uncert_W3J = getHistogram(tfile, "SG_W3J", variable,"_uncert")
        Uncert_W.Add(Uncert_W2J)
        Uncert_W.Add(Uncert_W3J)

        Uncert_TT = getHistogram(tfile, "SG_TT", variable,"_uncert")

        Uncert_ZLL = getHistogram(tfile, "SG_ZLL", variable,"_uncert")
        ...
        # Data
        Uncert_data = getHistogram(tfile, "SG_dataRunB", variable,"_uncert")
        Uncert_dataRunC = getHistogram(tfile, "SG_dataRunC", variable,"_uncert")
        Uncert_data.Add(Uncert_dataRunC)

        # Data-driven QCD estimation
        Uncert_QCD = getHistogram(tfile, "SG_dataRunB", variable,"_uncert")
        Uncert_QCDRunC = getHistogram(tfile, "SG_dataRunC", variable,"_uncert")
    ...
    if variable == "svd" or variable == "nsv" or variable == "ppoint" or variable == "BSdChi2norm":
        c.SetLogy()
        c.SaveAs("%s.pdf" % ("PDF/"+variable+"_6_SG"))
        c.SaveAs("%s.png" % ("PNG/"+variable+"_6_SG"))
    else:
        c.SaveAs("%s.pdf" % ("PDF/"+variable+"_6_SG"))
        c.SaveAs("%s.png" % ("PNG/"+variable+"_6_SG"))
