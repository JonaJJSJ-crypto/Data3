...
def main(variable):
    ...
    LW200 = getHistogram(tfile, "DRC_LW200", variable)

    W = getHistogram(tfile, "DRC_W1J", variable)
    W2J = getHistogram(tfile, "DRC_W2J", variable)
    W3J = getHistogram(tfile, "DRC_W3J", variable)
    W.Add(W2J)
    W.Add(W3J)

    TT = getHistogram(tfile, "DRC_TT", variable)

    ZLL = getHistogram(tfile, "DRC_ZLL", variable)

    #WW = getHistogram(tfile, "DRC_WW", variable)

    #WZ = getHistogram(tfile, "DRC_WZ", variable)

    #ttZ = getHistogram(tfile, "DRC_ttZ", variable)

    # Data
    data = getHistogram(tfile, "DRC_dataRunB", variable)
    dataRunC = getHistogram(tfile, "DRC_dataRunC", variable)
    data.Add(dataRunC)

    # Data-driven QCD estimation
    QCD = getHistogram(tfile, "DRC_dataRunB", variable, "_cr")
    QCDRunC = getHistogram(tfile, "DRC_dataRunC", variable, "_cr")
    QCD.Add(QCDRunC)
    for name in ["DRC_W1J", "DRC_W2J", "DRC_W3J", "DRC_TT", "DRC_ZLL"]:
...
#    for x in [TT, W, ZLL]:#, WW, WZ, ttZ]:
    ...
    if variable == "ppoint":
        # Simulation
        #ggH = getHistogram(tfile, "ggH", variable)
        Uncert_LW200 = getHistogram(tfile, "DRC_LW200", variable,"_uncert")

        Uncert_W = getHistogram(tfile, "DRC_W1J", variable,"_uncert")
        Uncert_W2J = getHistogram(tfile, "DRC_W2J", variable,"_uncert")
        Uncert_W3J = getHistogram(tfile, "DRC_W3J", variable,"_uncert")
        Uncert_W.Add(Uncert_W2J)
        Uncert_W.Add(Uncert_W3J)

        Uncert_TT = getHistogram(tfile, "DRC_TT", variable,"_uncert")

        Uncert_ZLL = getHistogram(tfile, "DRC_ZLL", variable,"_uncert")
        ...
        # Data
        Uncert_data = getHistogram(tfile, "DRC_dataRunB", variable,"_uncert")
        Uncert_dataRunC = getHistogram(tfile, "DRC_dataRunC", variable,"_uncert")
        Uncert_data.Add(Uncert_dataRunC)

        # Data-driven QCD estimation
        Uncert_QCD = getHistogram(tfile, "DRC_dataRunB", variable,"_uncert")
        Uncert_QCDRunC = getHistogram(tfile, "DRC_dataRunC", variable,"_uncert")
    ...
    if variable == "svd" or variable == "nsv" or variable == "ppoint" or variable == "BSdChi2norm":
        c.SetLogy()
        c.SaveAs("%s.pdf" % ("PDF/"+variable+"_5_DRC"))
        c.SaveAs("%s.png" % ("PNG/"+variable+"_5_DRC"))
    else:
        c.SaveAs("%s.pdf" % ("PDF/"+variable+"_5_DRC"))
        c.SaveAs("%s.png" % ("PNG/"+variable+"_5_DRC"))
