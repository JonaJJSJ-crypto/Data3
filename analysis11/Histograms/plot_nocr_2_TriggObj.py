...
def main(variable):
    ...
    LW200 = getHistogram(tfile, "TO_LW200", variable)

    W = getHistogram(tfile, "TO_W1J", variable)
    W2J = getHistogram(tfile, "TO_W2J", variable)
    W3J = getHistogram(tfile, "TO_W3J", variable)
    W.Add(W2J)
    W.Add(W3J)

    TT = getHistogram(tfile, "TO_TT", variable)

    ZLL = getHistogram(tfile, "TO_ZLL", variable)

    #WW = getHistogram(tfile, "TO_WW", variable)

    #WZ = getHistogram(tfile, "TO_WZ", variable)

    #ttZ = getHistogram(tfile, "TO_ttZ", variable)

    # Data
    data = getHistogram(tfile, "TO_dataRunB", variable)
    dataRunC = getHistogram(tfile, "TO_dataRunC", variable)
    data.Add(dataRunC)

    # Data-driven QCD estimation
    QCD = getHistogram(tfile, "TO_dataRunB", variable, "_cr")
    QCDRunC = getHistogram(tfile, "TO_dataRunC", variable, "_cr")
    QCD.Add(QCDRunC)
    for name in ["TO_W1J", "TO_W2J", "TO_W3J", "TO_TT", "TO_ZLL"]:
...
#    for x in [TT, W, ZLL]:#, WW, WZ, ttZ]:
    ...
    if variable == "ppoint":
        # Simulation
        #ggH = getHistogram(tfile, "ggH", variable)
        Uncert_LW200 = getHistogram(tfile, "TO_LW200", variable,"_uncert")

        Uncert_W = getHistogram(tfile, "TO_W1J", variable,"_uncert")
        Uncert_W2J = getHistogram(tfile, "TO_W2J", variable,"_uncert")
        Uncert_W3J = getHistogram(tfile, "TO_W3J", variable,"_uncert")
        Uncert_W.Add(Uncert_W2J)
        Uncert_W.Add(Uncert_W3J)

        Uncert_TT = getHistogram(tfile, "TO_TT", variable,"_uncert")

        Uncert_ZLL = getHistogram(tfile, "TO_ZLL", variable,"_uncert")
        ...
        # Data
        Uncert_data = getHistogram(tfile, "TO_dataRunB", variable,"_uncert")
        Uncert_dataRunC = getHistogram(tfile, "TO_dataRunC", variable,"_uncert")
        Uncert_data.Add(Uncert_dataRunC)

        # Data-driven QCD estimation
        Uncert_QCD = getHistogram(tfile, "TO_dataRunB", variable,"_uncert")
        Uncert_QCDRunC = getHistogram(tfile, "TO_dataRunC", variable,"_uncert")
    ...
    if variable == "svd" or variable == "nsv" or variable == "ppoint" or variable == "BSdChi2norm":
        c.SetLogy()
        c.SaveAs("%s.pdf" % ("PDF/"+variable+"_2_TO"))
        c.SaveAs("%s.png" % ("PNG/"+variable+"_2_TO"))
    else:
        c.SaveAs("%s.pdf" % ("PDF/"+variable+"_2_TO"))
        c.SaveAs("%s.png" % ("PNG/"+variable+"_2_TO"))
