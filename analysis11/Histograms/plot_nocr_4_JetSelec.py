...
def main(variable):
    ...
    LW200 = getHistogram(tfile, "JetS_LW200", variable)

    W = getHistogram(tfile, "JetS_W1J", variable)
    W2J = getHistogram(tfile, "JetS_W2J", variable)
    W3J = getHistogram(tfile, "JetS_W3J", variable)
    W.Add(W2J)
    W.Add(W3J)

    TT = getHistogram(tfile, "JetS_TT", variable)

    ZLL = getHistogram(tfile, "JetS_ZLL", variable)

    #WW = getHistogram(tfile, "JetS_WW", variable)

    #WZ = getHistogram(tfile, "JetS_WZ", variable)

    #ttZ = getHistogram(tfile, "JetS_ttZ", variable)

    # Data
    data = getHistogram(tfile, "JetS_dataRunB", variable)
    dataRunC = getHistogram(tfile, "JetS_dataRunC", variable)
    data.Add(dataRunC)
...
#    for x in [TT, W, ZLL]:#, WW, WZ, ttZ]:
    ...
    if variable == "ppoint":
        # Simulation
        #ggH = getHistogram(tfile, "ggH", variable)
        Uncert_LW200 = getHistogram(tfile, "JetS_LW200", variable,"_uncert")

        Uncert_W = getHistogram(tfile, "JetS_W1J", variable,"_uncert")
        Uncert_W2J = getHistogram(tfile, "JetS_W2J", variable,"_uncert")
        Uncert_W3J = getHistogram(tfile, "JetS_W3J", variable,"_uncert")
        Uncert_W.Add(Uncert_W2J)
        Uncert_W.Add(Uncert_W3J)

        Uncert_TT = getHistogram(tfile, "JetS_TT", variable,"_uncert")

        Uncert_ZLL = getHistogram(tfile, "JetS_ZLL", variable,"_uncert")
        ...
        # Data
        Uncert_data = getHistogram(tfile, "JetS_dataRunB", variable,"_uncert")
        Uncert_dataRunC = getHistogram(tfile, "JetS_dataRunC", variable,"_uncert")
        Uncert_data.Add(Uncert_dataRunC)

        # Data-driven QCD estimation
        Uncert_QCD = getHistogram(tfile, "JetS_dataRunB", variable,"_uncert")
        Uncert_QCDRunC = getHistogram(tfile, "JetS_dataRunC", variable,"_uncert")
    ...
    if variable == "svd" or variable == "nsv" or variable == "ppoint" or variable == "BSdChi2norm":
        c.SetLogy()
        c.SaveAs("%s.pdf" % ("PDF/"+variable+"_4_JetS"))
        c.SaveAs("%s.png" % ("PNG/"+variable+"_4_JetS"))
    else:
        c.SaveAs("%s.pdf" % ("PDF/"+variable+"_4_JetS"))
        c.SaveAs("%s.png" % ("PNG/"+variable+"_4_JetS"))
