...
def main(variable):
    ...
    LW200 = getHistogram(tfile, "HQ_LW200", variable)

    W = getHistogram(tfile, "HQ_W1J", variable)
    W2J = getHistogram(tfile, "HQ_W2J", variable)
    W3J = getHistogram(tfile, "HQ_W3J", variable)
    W.Add(W2J)
    W.Add(W3J)

    TT = getHistogram(tfile, "HQ_TT", variable)

    ZLL = getHistogram(tfile, "HQ_ZLL", variable)

    #WW = getHistogram(tfile, "HQ_WW", variable)

    #WZ = getHistogram(tfile, "HQ_WZ", variable)

    #ttZ = getHistogram(tfile, "HQ_ttZ", variable)

    # Data
    data = getHistogram(tfile, "HQ_dataRunB", variable)
    dataRunC = getHistogram(tfile, "HQ_dataRunC", variable)
    data.Add(dataRunC)

    # Data-driven QCD estimation
    QCD = getHistogram(tfile, "HQ_dataRunB", variable, "_cr")
    QCDRunC = getHistogram(tfile, "HQ_dataRunC", variable, "_cr")
    QCD.Add(QCDRunC)
    for name in ["HQ_W1J", "HQ_W2J", "HQ_W3J", "HQ_TT", "HQ_ZLL"]:
...
#    for x in [TT, W, ZLL]:#, WW, WZ, ttZ]:
    ...
    if variable == "ppoint":
        # Simulation
        #ggH = getHistogram(tfile, "ggH", variable)
        Uncert_LW200 = getHistogram(tfile, "HQ_LW200", variable,"_uncert")

        Uncert_W = getHistogram(tfile, "HQ_W1J", variable,"_uncert")
        Uncert_W2J = getHistogram(tfile, "HQ_W2J", variable,"_uncert")
        Uncert_W3J = getHistogram(tfile, "HQ_W3J", variable,"_uncert")
        Uncert_W.Add(Uncert_W2J)
        Uncert_W.Add(Uncert_W3J)

        Uncert_TT = getHistogram(tfile, "HQ_TT", variable,"_uncert")

        Uncert_ZLL = getHistogram(tfile, "HQ_ZLL", variable,"_uncert")
        ...
        # Data
        Uncert_data = getHistogram(tfile, "HQ_dataRunB", variable,"_uncert")
        Uncert_dataRunC = getHistogram(tfile, "HQ_dataRunC", variable,"_uncert")
        Uncert_data.Add(Uncert_dataRunC)

        # Data-driven QCD estimation
        Uncert_QCD = getHistogram(tfile, "HQ_dataRunB", variable,"_uncert")
        Uncert_QCDRunC = getHistogram(tfile, "HQ_dataRunC", variable,"_uncert")
