...
TH1F*  WZelectron_dxy = new TH1F(" WZelectron_dxy","Electrons pt for missing trigg",100,-0.5,0.5);
TH1F*  WZelectron_dz = new TH1F(" WZelectron_dz","Electrons pt for missing trigg",200,-10,10);
...
TH1F*  WZSGenele_dxy = new TH1F(" WZSGenele_dxy","Signal Genele pt ",100,-0.5,0.5);
TH1F*  WZSGenele_dz = new TH1F(" WZSGenele_dz","Signal Genele pt ",200,-10,10);
...
TH1F*  WZGenelec_disp = new TH1F("Generated_electron_Bmatch","Gen Electron secondary vertex displacement",200,0,1);
TH1F*  WZEleBsecvec = new TH1F("Electron_Bmatch","Best matched secondary vertex displacement",200,0,1);
TH1F*  WZGenelec_disp_f = new TH1F("Generated_electron_Bmatch Filtered","Gen Electron secondary vertex displacement filtered",200,0,1);
TH1F*  WZEleBsecvec_f = new TH1F("Electron_Bmatch_Filtered","Best matched secondary vertex displacement filtered",200,0,1);
TH1F*  WZTrigObj = new TH1F("TriggerObject_Electrons","Number of electrons and trigger Object matching",2,0,2);
TH1F*  WZTrigObjDR = new TH1F("TriggerObject_Electrons_DR","Number of electrons and trigger Object matching deltaR",200,0,4);
...
EventLoopAnalysisTemplate::EventLoopAnalysisTemplate(TString thefile, TString thelabel, Float_t sampleweight) : fChain(0)
{
  ...
  hists[3]= WZSGenele_num;
  hists[4]= WZSGenele_pt;
  hists[5]= WZSGenele_eta;
  hists[6]= WZGenZdau_pt;
  hists[7]= WZGenZdau_eta;
  hists[8]= WZGenelec_num;
  hists[9]= WZGenelec_pt;
  hists[10]= WZGenelec_eta;
  hists[11]= WZGenelec_disp;
  hists[12]= WZEleBsecvec;
...
//Filling Electrons pt
      ...
      for (size_t j = 0; j < GenPart_pt->size(); j++) {
        if(abs(GenPart_pdgId->at(j)) == 11 && abs(GenPart_mompdgId->at(j)) == 556){
          if( genelec_DRscore->at(i)<0.1 && genelec_pt->at(i)==GenPart_pt->at(j) ){
            hists[1]->Fill(electron_pt->at(i));
             WZelectron_dxy->Fill(electron_dxy->at(i));

            if(abs(electron_dxy->at(i))<0.02){
               WZSGenele_dxy->Fill(electron_dxy->at(i));
              ...
              for (size_t y=0; y<trigobj_px->size(); y++){
               ...
               WZTrigObjDR->Fill(saveDR);

              if (saveDR<0.2)  WZTrigObj->Fill(1);
              else  WZTrigObj->Fill(0);
            }
  ...
  if(electron_pt->size() == 0){
    //cout<<"Aqui\n";
     WZGenelec_disp_f->Fill(-1);
     WZEleBsecvec_f->Fill(-1);
  }
  else{
    //cout<<"Aqui no\n";
    for (size_t i = 0; i < electron_pt->size(); i++) {
      ...
      for (size_t i = 0; i < electron_pt->size(); i++) {
        ...
        for (size_t j = 0; j < GenPart_pt->size(); j++) {
          if(abs(GenPart_pdgId->at(j)) == 11 && abs(GenPart_mompdgId->at(j)) == 556){
            if( genelec_DRscore->at(i)<0.1 && genelec_pt->at(i)==GenPart_pt->at(j) ){
              if(electron_Bsecvec->at(i)!=-1){
                if(secvec_disp->size()>electron_Bsecvec->at(i))  WZEleBsecvec_f->Fill(secvec_disp->at(electron_Bsecvec->at(i)));
              }
            }
          }
        }
 ...
 */
  ...
  //Save signal region histos
   ...
   WZGenelec_pt->Write();
   WZGenelec_eta->Write();
   WZGenelec_disp->Write();
   WZEleBsecvec->Write();
   WZGenelec_disp_f->Write();
   WZEleBsecvec_f->Write();
   WZTrigObj->Write();
   WZTrigObjDR->Write();
