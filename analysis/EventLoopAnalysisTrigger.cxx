        ...
        for(size_t i=0; i < jet_e->size(); i++){
          for(size_t k=0; k < jet_e->size(); k++){
            if(i!=k){
              ...
              //corr_pt=corr_jet_pt->at(i)+corr_jet_pt->at(k);x`x`
              ...
              pz=jet_pz->at(i)+jet_pz->at(k);
              e=jet_e->at(i)+jet_e->at(k);
        ...
        for(size_t i=0; i < jet_e->size(); i++){
          for(size_t k=0; k < jet_e->size(); k++){
            for(size_t l=0; l < electron_e->size(); l++){
              px=0,py=0,pz=0,e=0,m=0;
              //cout<<mass1+jet_mass->at(j)<<endl;
              //corr_pt=corr_jet_pt->at(i)+corr_jet_pt->at(k);
              ...
              pz=jet_pz->at(i)+jet_pz->at(k)+electron_pz->at(l);
              e=jet_e->at(i)+jet_e->at(k)+electron_e->at(l);
              m=sqrt( e*e - px*px - py*py - pz*pz );
